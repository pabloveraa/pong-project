...
img_new = [];  %new image to compute optical flow
...
%-------pointTracker--------------------------
%move the paddles using the optical flow
    function pointTracker(k2)
        img_new = captureImage(k2);
        img_width = size(img_new,2);
        thr_pad = 5;  %minimum displacement in pixels to move the paddles
        if( not(isempty(img_prev)) && not(isempty(img_new)) )
            pTracker = vision.PointTracker('MaxIterations',5);
            points = detectMinEigenFeatures(rgb2gray(img_prev),'MinQuality',0.05);
%             points = detectSURFFeatures(rgb2gray(img_prev));
            pts1 = points.Location;
            initialize(pTracker,pts1, img_prev);
            [pts2,ind_valid] = step(pTracker, img_new);
            pts1 = pts1(ind_valid,:);
            pts2 = pts2(ind_valid,:);
            ind_left = (pts2(:,1) <= 0.5*img_width);
            ind_right = (pts2(:,1) >= 0.5*img_width);
            dy_left = pts2(ind_left,2) - pts1(ind_left,2);
            dy_right = pts2(ind_right,2) - pts1(ind_right,2);
            dy_left = dy_left(abs(dy_left)>0.1);
            dy_right = dy_right(abs(dy_right)>0.1);
            if( not(isempty(dy_left)) )
                dym = mean(dy_left);
                paddle1V = double(dym>thr_pad) - double(dym<(-thr_pad));
            else
                paddle1V = 0;
            end
            if( not(isempty(dy_right)) )
                dym = mean(dy_right);
...
%-------------captureImage----------------
    function img_new = captureImage(k2)
        ...
        if( validData )
            color = k2.getColor;
            img_new = imresize(color,0.25,'nearest');
        else
            img_new = [];
        end
    end
...
createFigure;
newGame;
k2 = Kin2('color');  %initialize kinect color camera
pause(5)
k =0;
profile on
tic;
while ~quitGame
  ...
  if(not(isempty(img_new)))
      imwrite(img_new,sprintf('img%03d.jpg',k));
  end
  refreshPlot;
  checkGoal;
  k = k + 1;
