...
int capture_depth(Mat imdepth, int nimg){
  char fn[100];
  sprintf(fn, "../images/depth%04d.png", nimg);
  Mat imd = imread(fn, CV_LOAD_IMAGE_ANYDEPTH);
  imd.convertTo(imdepth, CV_32F, 1.0, 0.0);
  return 1;
}

//=====================================================================
//capture a color image with the kinect
int capture_color(Mat imcolor, int nimg){
  char fn[100];
  sprintf(fn, "../images/color%04d.jpg", nimg);
  Mat img = imread(fn);
  img.copyTo(imcolor);
  return 1;
}
