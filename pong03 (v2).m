...
%move the paddles using the optical flow
    function pointTracker(k2)
        ...
        thr_pad = 10;  %minimum displacement in pixels to move the paddles
        [xn,yn] = head_centers(img_new);
        if( not(isempty(xn)) && not(isempty(xp)) )
            dy_left = yn(1) - yp(1);
            dy_right = yn(2) - yp(2);
            paddle1V = (dy_left>=thr_pad) - (dy_left<=(-thr_pad));
            paddle2V = (dy_right>=thr_pad) - (dy_right<=(-thr_pad));
        end
        xp = xn;
        yp = yn;
    end
...
%---------------head_centers----------------------
    function [xc,yc] = head_centers(img)
        max_depth = 2000;
        thr_head = 100;
        scale = 0.5;
        if( not(isempty(img)) )
            ...
            cols = size(img,2);
            imb = (img>0) & (img<max_depth);
            imb = imerode(imb,ones(3,3));
            xm = floor(0.5*cols);
            [xleft,yleft] = head_median_location(img(:,1:xm),imb(:,1:xm),thr_head);
            [xright,yright] = head_median_location(img(:,xm+1:cols),imb(:,xm+1:cols),thr_head);
            xc = [xleft; xright+xm]/scale;
            yc = [yleft; yright]/scale;
        end
        
%--------head_median_location------------------------------------
    function [xm,ym] = head_median_location(img,imb,thr_head)
        min_points = 100;
        if( sum(imb)>=min_points )
            mn = min(img(imb));
            ims = (img>=mn) & (img<=(mn+thr_head));
            ims = (ims & imb);
            [y,x] = find(ims);
            xm = median(x);
            ym = median(y);
...
createFigure;
newGame;
k2 = Kin2('depth');  %initialize kinect color camera
while ~quitGame
  moveBall;
  pointTracker(k2);
  movePaddles;
