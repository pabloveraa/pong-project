...
MIN_BALL_SPEED = 5; %each round ball starts at this speed
MAX_BALL_SPEED = 30; %wont accelerate bast this, dont set too high or bugs.
...
%-------floorDetection-----------------------------------------------
%detect the floor in the first image to find the rotation matrix that is
%used to correct the floor slope
    function floorDetection(k2)
       img_new = [];
       while(not(isempty(img_new))) 
           img_new = captureImage(k2); %Captura la imagen de profundidad, si no hay espera 1s 
           pause(1);
       end
       [rows,cols] = size(img_new);
        ...
        rotation_matrix = [cosd(thy) 0 sind(thy); -sind(thx)*sind(thy) cosd(thx) sind(thx)*cosd(thy); ...
        ...
        depth_max = floor_depth - ftol;
    end


%-------pointTracker-----------------------------------------------
%move the paddles using body movements of two persons detected by the
%kinect depth camera
    function pointTracker(k2)
        img_new = captureImage(k2);
        thr_pad = 50;  %minimum displacement in mm to move the paddles
        R = rotation_matrix;
        ...
        if(not(isempty(img_new)))
            ...
            %3D coordinates using the camera reference system
            [um,vm] = meshgrid(1:cols,1:rows);
            Z = img_new(:);
            X = Z.*(um(:)-cc(1))/f;
            Y = Z.*(vm(:)-cc(2))/f;
            %rotate Z and Y coordinates to correct the floor slope
            Zr = R(3,1)*X + R(3,2)*Y + R(3,3)*Z;
            Yr = R(2,1)*X + R(2,2)*Y + R(2,3)*Z;           
            %minimum depth locations on the blobs at each side of the image
            Zr(Zr>depth_max | Zr==0) = inf;
            Z_left = Zr;
            Z_left(:,round(0.5*cols):cols) = inf;
            [v,u] = find(Z_left==min(Z_left(:)));
            yleft = Yr(v(1),u(1));
            Z_right = Zr;
            Z_right(:,1:round(0.5*cols)) = inf;
            [v,u] = find(Z_right==min(Z_right(:)));
            yright = Yr(v(1),u(1));
            if( yleft<inf && yright<inf )
                loc_new = [yleft, yright];
                if(not(isempty(loc_prev)))
                    dym = loc_new(1)-loc_prev(1);
                    paddle1V = double(dym>thr_pad) - double(dym<(-thr_pad));
                    dym = loc_new(2)-loc_prev(2);
                    paddle2V = double(dym>thr_pad) - double(dym<(-thr_pad));
                else
                    paddle1V = 0;
                    paddle2V = 0;
                end
...
%-------------captureImage----------------
    function img_new = captureImage(k2)
        validData = k2.updateData;
        if( validData )
            depth = k2.getDepth; % Capture the image depth (toma imagen a profundidad)
            img_new = imresize(depth,0.25,'nearest'); % Image to 1/4 of total size
        else
            img_new = [];
        end
    end
...
while ~quitGame
  moveBall;
  % the pointTracker function allows to move the paddles using body
  % movemenst seen by the kinect camera
  pointTracker(k2);
  movePaddles;
  refreshPlot;
  checkGoal;
