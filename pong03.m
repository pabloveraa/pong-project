...
% captureColorImage : capture an image from the kinect color camera
% runme : run the game
%
% this program requieres:
...
classdef pong03 < handle
    properties(Constant)
        ...
        MAX_BALL_SPEED = 10; %wont accelerate bast this, dont set too high or bugs.
    ...
    methods
        ...
        function floorDetection(obj,k2)
            ...
            while(isempty(img_new))
                img_new = obj.captureDepthImage(k2); %capture depth image
                pause(1);
            end
        ...
        function pointTracker(obj,k2)
            img_new = obj.captureDepthImage(k2);
...
%             Zd = (d - nx*Xd - ny*Yd)/nz;
        ...
        function scalePlayers(obj,ud,vd,cols,rows)
            ...
            k1 = obj.PLOT_H / (max(Yleft) - min(Yleft));
            b1 = -k1 * min(Yleft);
            %scale and offset for the player 2
            k2 = obj.PLOT_H / (max(Yright) - min(Yright));
            b2 = -k2 * min(Yright);
        ...
        function img_new = captureDepthImage(obj,k2)
            validData = k2.updateData;
            if( validData )
                depth = k2.getDepth; % Capture a depth image
                %resize image a half of total size
                img_new = double( imresize(depth,0.5,'nearest') );
%                 img_new = imrotate(img_new,180,'nearest');
            else
                img_new = [];
            end
        end
        ...
        %======================================================
        %capture an image from the kinect color camera
        function img_color = captureColorImage(obj,k2)
            validData = k2.updateData;
            if( validData )
                img_color = k2.getColor; % Capture a color image
            else
                img_color = [];
            end
        end        
        ...
        %run the game
        function runme(obj)
            k2 = Kin2('depth','color');  %initialize kinect depth camera
            obj.floorDetection(k2);  %detect the floor
            
            %take a color and depth images to set the game area            
            img_depth = [];
            img_color= [];
            while(isempty(img_depth) || isempty(img_color))
                %capture a depth and a color images
                img_depth = obj.captureDepthImage(k2);
                img_color = obj.captureColorImage(k2);
                pause(1);
            end
            %set the game area
