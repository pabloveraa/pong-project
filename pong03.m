...
% added variables
prevY = [0,0]; %s: initial value for Ys
...
%called from main loop on every frame
  function moveBall
    ...
    newX = ballX + 0.5*(ballSpeed * ballVector(1));
    newY = ballY + 0.5*(ballSpeed * ballVector(2));
...
%kinect depth camera
    function pointTracker(k2)
        ...
        if(not(isempty(img_new)))
           ...
           figure(2); hold off;
           imshow(imb); hold on;
           plot(median(um(iLeft)),median(vm(iLeft)),'ro','linewidth',2);
           plot(median(um(iRight)),median(vm(iRight)),'ro','linewidth',2);
           
           if (not(isempty(iLeft)) && not(isempty(iRight)))
               mYLeft = median(Yr(iLeft));
               mYRight = median(Yr(iRight + round(0.5*cols*rows)));
               
               loc_new = [mYLeft, mYRight];
...
%         depth = imread(dImPNG(k).name);
%         color = imread(dImJPG(k).name);
       if( validData )
           depth = k2.getDepth; % Capture the image depth (toma imagen a profundidad)
            img_new = imresize(depth,0.5,'nearest'); % Image to 1/4 of total size
            
            %imwrite(img_new, sprintf('ir%06d.png',k));
            %color = k2.getColor;
%             color_new = imresize(color, 0.5,'nearest');
           % imwrite(color_new, sprintf('ir%06d.jpg',k));
...
while ~quitGame
  ...
  % movement seen by the kinect camera
  pointTracker(k2);
  movePaddles;
  refreshPlot;
  checkGoal;
