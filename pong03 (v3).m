...
%kinect depth camera
    function pointTracker(k2)
        ...
        thr_pad = 5;  %minimum displacement in pixels to move the paddles
        R = rotation_matrix;
        ...
        if( not(isempty(loc_prev)) && not(isempty(img_new)))
           Z = img_new(:);
           X = Z.*(um(:)-cc(1))/f;
           Y = Z.*(vm(:)-cc(2))/f;
           %segment image regions above the floor
            ...
            [vleft,~] = find(imb(:,1:round(0.5*cols)));
            %median of the blob coordinates at the right
            [vright,~] = find(imb(:,round(0.5*cols):cols));
            
            if (not(isempty(vleft)) && not(isempty(vright)))
                loc_new = median([vleft,vright]); 
